...
const update = ({acceleration, velocity, position, mass}, delta, canvas) => {
    ...
    position[1] = position[0]+ velocity[1] * delta;
    if(position[0]<0) position[0] = position[0] + canvas.width;
    if(position[0]>canvas.width) position[0] = position[0]-canvas.width;
    if(position[1]<0) position[1] = position[1]+canvas.width;
    if(position[1]>canvas.width) position[1] = position[1]-canvas.width;
